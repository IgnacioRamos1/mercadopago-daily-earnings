...
def create_db_engine(stage):
    if 'AWS_EXECUTION_ENV' in os.environ:
        ...
        HOST = get_parameter(f'{stage}_db_host_mp')
        PORT = get_parameter('db_port')
        DATABASE = get_parameter(f'{stage}_db_name_mp')
        USER = get_parameter(f'{stage}_db_username_mp')
        PASSWORD = get_parameter(f'{stage}_db_password_mp')
    else:
